...
/* There are several types of files that go in the spool directory,
   ...
   communication occurs.  Whenever the system name LOCAL appears
   below, it means whatever the local system name is.
...
/* Given the name of a file as specified in a UUCP command, and the
   ...
   file), D. (a data file) or X. (an execution file).  */
     ...
     const char *zsystem;
{
  ...
  switch (*zsimple)
	...
	if (c > 0)
	  {
	    ...
	    if (! ftruncated || fsysdep_directory (zalloc))
	      return zsysdep_in_dir (zalloc, zsimple);
	  }
	...
	return zsappend3 (zsystem, "D.", zsimple);
	 ...
	 systems will be using either HDB or TAYLOR.  */
    case 'X':
...
#endif /* ! SPOOLDIR_HDB && ! SPOOLDIR_TAYLOR */
