 ...
 */
static int izrecv_hdr P((struct sdaemon *qdaemon, achdrval_t hdr));	/* IE: zgethdr() */
...
char **pzlog;
{
	*pzlog = zbufalc (sizeof "protocol 'a' starting: , , , , , " + 100);
	...
	if (cZtx_window % 1024 != 0 ||
		...
		ulog (LOG_ERROR, "fzstart: cZtx_window not one of 4096, 8192, 16384, 32768, 65536");
		return FALSE;
	}
...
struct sdaemon *qdaemon;
{
	...
	while (fzreceive_ready ()) {
		...
		case ZPAD:
			...
			DEBUG_MESSAGE0 (DEBUG_PROTO, "fzprocess: possible ZRPOS packet");
...
struct sdaemon *qdaemon;
{
	...
	for (i = 0; i < cZstartup_retries; i++) {
		...
		DEBUG_MESSAGE0 (DEBUG_PROTO, "fzstart_proto: Protocol started");
		return TRUE;
...
long *plredo;
{
	...
	*plredo = -1;
	...
	for (;;) {
		...
		case ZM_TIMEOUT:
			if (--cerr < 0) {
				ulog (LOG_ERROR, "fzfinish_tx: retries exhausted");
				return FALSE;
			}
			/*
...
boolean flag;
{
	...
	for (;;) {
		...
		case ZM_TIMEOUT:
			if (--cerr < 0) {
				ulog (LOG_ERROR, "getinsync: retries exhausted");
				return ZM_ERROR;
			}
